...
modality = 'j'
graph = 'sign_language'
work_dir = f'./work_dirs/signlanguage_msasl/100_joint_{datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")}'
load_from = ""
...
model = dict(
    ...
    cls_head=dict(type='SimpleHead', joint_cfg='sign_language', num_classes=100, in_channels=384, weight=1.0))

dataset_type = 'PoseDataset'
ann_file = "./data/signlanguage_msasl/100msasl_focushand_w_face.pkl"
...
val_pipeline = [
    dict(type='UniformSampleFrames', clip_len=80, num_clips=1),
    dict(type='PoseDecode'),
    dict(type='GenSkeFeat',dataset = 'sign_language', feats=[modality]),
    dict(type='FormatGCNInput', num_person=1),
...
test_pipeline = [
    dict(type='UniformSampleFrames', clip_len=80, num_clips=10),
    dict(type='PoseDecode'),
    dict(type='GenSkeFeat',dataset = 'sign_language', feats=[modality]),
    dict(type='FormatGCNInput', num_person=1),
    dict(type='Collect', keys=['keypoint', 'label'], meta_keys=[]),
...
optimizer = dict(type='SGD', lr=3e-4, momentum=0.9, weight_decay=0.0005, nesterov=True)
