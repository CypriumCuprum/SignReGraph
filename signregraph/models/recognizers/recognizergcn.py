...
class RecognizerGCN(BaseRecognizer):
    ...
    def forward_train(self, keypoint, label, drop_area, **kwargs):
        ...
        losses = dict()
        x, hand_pred = self.extract_feat(keypoint)
        
        cls_score = self.cls_head(x)
        gt_label = label.squeeze(-1)
        loss = self.cls_head.loss(cls_score, hand_pred, gt_label, drop_area)
        losses.update(loss)
